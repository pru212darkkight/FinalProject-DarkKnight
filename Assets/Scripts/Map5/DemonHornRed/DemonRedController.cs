...
public class DemonRedController : MonoBehaviour
{
    [Header("References")]
    ...
    public DemonRedAttack miniBossAttack;
    ...
    private void Awake()
    {
        ...
        miniBossAttack = GetComponent<DemonRedAttack>();
    ...
    void LookAtTarget(float targetX)
    {
        ...
        if (dir > 0 && transform.localScale.x > 0) // đang nhìn trái, cần nhìn phải (scale.x âm)
        {
            Vector3 scale = transform.localScale;
            scale.x *= -1;
            transform.localScale = scale;
        }
        else if (dir < 0 && transform.localScale.x < 0) // đang nhìn phải, cần nhìn trái (scale.x dương)
        {
            Vector3 scale = transform.localScale;
            scale.x *= -1;
            transform.localScale = scale;
